...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_profile'])) {
        ...
        if (empty($error)) {
            $stmt = $db->prepare("UPDATE utilisateurs SET nom = :nom, prenom = :prenom, email = :email, age = :age, poids = :poids, taille = :taille, niveau = :niveau WHERE id_utilisateur = :id");
    ...
    elseif (isset($_POST['upload_photo']) && isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
...
<div class="mb-8">
    ...
    <p class="text-gray-600">Modifiez vos informations personnelles et votre mot de passe.</p>
</div>
    ...
    <div class="lg:col-span-2 bg-white rounded-xl shadow-md p-6">
        ...
        <form method="POST">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-bold mb-1">Poids (kg)</label>
                    <input type="number" step="0.1" name="poids" value="<?php echo $user['poids']; ?>" class="w-full px-3 py-2 border rounded-lg">
                </div>
                <div>
                    <label class="block text-sm font-bold mb-1">Taille (cm)</label>
                    <input type="number" step="0.1" name="taille" value="<?php echo $user['taille']; ?>" class="w-full px-3 py-2 border rounded-lg">
                </div>
                <div>
                    <label class="block text-sm font-bold mb-1">Niveau</label>
